...
public class Driver {
    ...
    // Arrays for name of each button for each page, include OPTIONS which is on all
    // pages
    ...
    // Current page user is on, I forget why but there's an important reason I had
    // to make this
    public static Page current;
    // Used for ensuring no weirdness happens with typing filenames
    public static char[] illegalKeys = { '/', '\\', '|', '"', '*', '<', '>', ':', '?' };
    // When switching between pages, these variables keep track of board and buttons
    // on current page
    ...
    public static Board createBoard = new Board(50, 50, 20, 20, DEAFULT_ROWS_AND_COLS, DEAFULT_ROWS_AND_COLS, false,
            new boolean[DEAFULT_ROWS_AND_COLS][DEAFULT_ROWS_AND_COLS]);
    ...
    public static void initializeElements() {
        ...
        for (int i = 0; i < constructors.length; i++) {
            constructors[i] = new Button(50, 60 - 2 * (5 + 2 * BTN_HALFHEIGHT) * i, 3 * BTN_HALFWIDTH,
                    2 * BTN_HALFHEIGHT, constructorNames[i], true);
        }
    ...
    public static void run(Button btn) {
        if (inArray(btn.name, optionNames)) {
            ...
            switch (btn.name) {
                ...
                case "Is Alive":
                    methodText.text = (game.isAlive()) ? "There are still living cells."
                    ...
                    while (true) {
                        if (StdDraw.hasNextKeyTyped()) {
                            ...
                            if (48 <= keystroke && keystroke <= 57 && len < 9) {
                                steps = steps.concat(Character.toString((char) keystroke));
                ...
                case "Reset":
                    // Students better not make a file named "default.txt" or else it won't go back
                    // to it
                    game = (methodFilename.equals("default")) ? new GameOfLife()
                            : new GameOfLife(methodFilename + ".txt");
                    initializeMethod(game);
                    displayPage(Page.METHOD);
                    StdDraw.pause(DELAY);
                    break;
    ...
    // This method just initializes things in method w/ game object since that can't
    // be done
    // right at the start when it doesn't exist yet, this is just to cut down on
    // code
            ...
            if (file.isFile()) {
                ...
                try {
                    ...
                    if (filename.substring(indexOfExtension).equals(".txt") && filename.contains(beginning)) {
                        // Get number from file, so if current filename is "name_saved3.txt", this will
                        // get the "3"
                        String currentNumber = filename.substring(filename.indexOf(beginning) + beginning.length(),
                                indexOfExtension);
